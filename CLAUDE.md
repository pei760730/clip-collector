# CLAUDE.md — TeaBus-ClipBot 協作規則

> 接手這個 repo(含 AI)先讀這份。TeaBus-ClipBot = Telegram 短影音收集 bot,
> 取代舊 n8n 流程。貼「連結+備註」→ 解析→清理→判平台→抽 video ID→去重→**直接寫 TeaBus-VOC 的 Google Sheet「參考池」分頁**。
> (2026-06-22:廢「暫存區」中間層 —— TeaBus-VOC 已砍 sync-pool,bot 與 TeaBus-VOC 同表同 SA,直寫參考池就是最終狀態。)
...
| 找什麼 | 去哪 |
|---|---|
| 「參考池」欄位 / schema(SSOT) | `src/types.ts`:`RefRow` / `POOL_COLUMNS`(= TeaBus-VOC `schema.REFS` 5 欄)+ `PLATFORM_CODE`(顯示名→小寫碼) |
...
- **時區固定 `Asia/Taipei`**(`src/utils/date.ts`)。參考池「加入日期」用 ISO `YYYY-MM-DD`(`todayIsoTaipei`,對齊 TeaBus-VOC schema)。
- **寫入一律 RAW**(不用 USER_ENTERED),避免 video ID / 開頭 0 被吃成數字。
- **訊息純文字**,不用 MarkdownV2(舊版跳脫漏字會發送失敗)。
- **去重靠連結 key**(`dedupKey`):寫入參考池前讀現有「連結」欄,候選與既有列都用同一支推 key,重複就跳過。**全表比對、無時間窗**(參考池是 TeaBus-VOC 永久池,不像舊暫存區會 prune)。同支影片不同形態(youtu.be/watch?v=/shorts)收斂同 key;抓不到影片id 的退連結路徑 key。
...
## 第六層:與 TeaBus-VOC 對接契約(改欄位前先讀!跨 repo)

bot 是上游:**直接寫** Google 表「**短影音進度N**」(`<TeaBus-VOC sheet id｜待建>`,= TeaBus-VOC 的 `VOC_SPREADSHEET_ID`)的「**參考池**」分頁。
2026-06-22 起廢「暫存區」中間層 —— TeaBus-VOC 已砍 `sync.py` / `sync-pool`(暫存區→參考池 每日複製是純儀式,第一性原理刪除)。bot 與 TeaBus-VOC 同一張表、同一把 SA,bot 直寫參考池就是最終狀態。

- **同一張表**:bot `GOOGLE_SHEET_ID` 必須 = TeaBus-VOC `VOC_SPREADSHEET_ID`(`<TeaBus-VOC sheet id｜待建>`)。憑證共用 TeaBus-VOC 的 `service_account.json`(`<TeaBus-VOC 的 service account｜待建>`)。
- **參考池由 TeaBus-VOC 擁有,bot 不自建/不改表頭**:TeaBus-VOC `init-sheet` 建「參考池」。bot `GoogleSheetsStorage.ensureHeader` 只**驗表頭對齊**,缺分頁 / 表頭不齊一律 fail-fast(不替 TeaBus-VOC 動表結構,避免錯欄寫入靜默毀 TeaBus-VOC 的池)。
- **契約欄位 = TeaBus-VOC `schema.REFS` 5 欄(改名要兩 repo 一起)**。bot append 用固定欄序硬塞,**欄名 + 順序**都要對上;由 `tests/contract.test.ts` 守(改欄名 → CI 紅):
  - `id`:bot 寫**留空**(挑片時 GAS 搬待拍會發 T 號;參考池的 R 號 bot 不發)。
  - `平台`:**小寫碼**(`PLATFORM_CODE`:tiktok/youtube/facebook/instagram/threads/x/douyin/xiaohongshu;認不得 → `unknown`)。TeaBus-VOC 全系統用小寫碼。
...
- **NOTE/VIDEO_ID/SENDER 不進參考池**(TeaBus-VOC 設計如此,不是漏):參考池只存不可化約的 5 欄,梗/點子在搬進待拍後填「待拍.備註」。去重 key 寫入前由連結即時推導(`dedupKey`),不需存欄。
- **去重(寫入前,bot 端負責)**:`src/pipeline/index.ts` 的 `dedupKey` 對齊 TeaBus-VOC 舊 `sync._dedup_key` —— 優先「平台:影片id」(用 bot `detectPlatform`+`extractVideoId`,讓 youtu.be/watch?v=/shorts 收斂),抽不到才退連結路徑(砍 query/fragment + 去尾斜線 + lower)。`collect` 寫入前讀現有「連結」欄、候選與既有列同支推 key 比對,重複跳過。**全表比對、無時間窗**(參考池永久池,不 prune)。
...
- **挑片 = 在 Sheet 勾「挑」(bot 不參與)**:人在「參考池」勾「挑」checkbox → TeaBus-VOC 的 GAS `pickScan_`(onEdit simple trigger)即時把該列整列搬進待拍、發 T 號、刪參考池本列。bot 沒有挑片指令。
  ...
  - **沒有 `TeaBus-VOC pick` 指令**:搬移在 GAS(`pickScan_`),不是 Python CLI。別在文件/註解寫「TeaBus-VOC pick」(會誤導)。
- **平台偵測器兩套、各自獨立**:bot `detectPlatform`(hostname)與 TeaBus-VOC `parse_url`(regex)是兩份實作。bot 是參考池唯一寫入者 → 平台欄以 bot 判定為準(TeaBus-VOC 不再 re-derive)。`contract.test.ts` 釘住 bot 8 個平台碼都落在 TeaBus-VOC 認得的小寫碼集合。
- **改 TeaBus-VOC 一律另開 TeaBus-VOC session**,別從 bot 滑上游。
