...
4. **修 bug 前先想能不能用 schema / 設定 / 純函式 / 型別擋掉**,寫新 code 是最後手段。抽取/清理/分群規則的 SSOT 在 `@pei760730/collector-core` —— 要改去 core 改(先過 core 的 tests + dedupConformance),別在本 repo 憑印象重寫跑掉行為。(舊「n8n regex 1:1」紅線已退役 2026-07-03,與 `CLAUDE.md` §1-4 一致。)
...
npm run build         # tsc 出 dist/index.js(`npm start` 依賴它)
```
