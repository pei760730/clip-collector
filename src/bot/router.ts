...
import { HOT_VALUES } from "../types.js";
...
export function createBot(config: Config, storage: Storage, hooks?: BotHooks): Telegraf {
  ...
  // 但回一句「沒有權限」提示 —— 完全靜默會讓誤加的自己人以為 bot 壞了(要翻 Actions log 才撈得到 id)。
  ...
  if (config.allowedChatIds.length > 0) {
    ...
    bot.use(async (ctx, next) => {
      ...
      // 只留末 2 碼供粗略辨識重複來源(port 自 feed-collector router 的 maskId)。
  ...
  bot.command("stats", async (ctx) => {
    try {
      // reply 包 catch(照 handleCollectText 的護法):使用者封鎖 bot / chat 失效時 reply 丟例外,
      // 不該把「發送失敗」誤報成「取統計失敗」還驚動 error chat。統計本身失敗才走 catch。
  ...
  const handleCollectText = async (ctx: Context, text: string) => {
    ...
    try {
      ...
      // 收錄成功且 key 放得下 → 掛夯度按鈕,分享者一鍵下標。
      const kb =
        result.hotKey && hotKeyFits(result.hotKey) ? hotKeyboard(result.hotKey) : undefined;
  ...
  // 夯度按鈕點擊 → 回填該列「夯度」。callback_data = h:<idx>:<dedupKey>
  // (key 可能含 ":" 如 https://,故 (.+) 貪婪吃到尾;idx 在前所以不歧義)。
  bot.action(/^h:(\d+):(.+)$/, async (ctx) => {
    try {
      ...
      const ok = await storage.setHot(key, value);
      if (ok) {
        await ctx.answerCbQuery(`夯度:${value} ✓`).catch(() => {});
        // 在按鈕列把選中的標 ✅(視覺回饋,可再點換)。
        await ctx.editMessageReplyMarkup(hotKeyboard(key, idx).reply_markup).catch(() => {});
      } else {
        // 找不到 = 多半已勾「挑」搬去待拍了。
