...
describe("drain 單輪去重不 N+1(values.get 資料讀 O(1))", () => {
  it("一輪 5 筆不同連結收錄 → 資料 values.get 只打 1 次、append 5 次", async () => {
    ...
    for (const url of urls) {
      const r = await runCollect({ text: `${url} note` }, { storage, expandShortUrls: false });
  ...
  it("同輪稍後重複連結 → 命中 in-memory 索引、不再 append、仍不多打資料讀", async () => {
    ...
    const r1 = await runCollect({ text: `${url} 第一次` }, { storage, expandShortUrls: false });
    const r2 = await runCollect({ text: `${url} 又貼一次` }, { storage, expandShortUrls: false });
