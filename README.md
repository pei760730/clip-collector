...
> **緣起**：給客戶**紅茶巴士**的短影音收集 bot，從 [`short-video-bot`](https://github.com/pei760730/short-video-bot) 複製改造（姊妹源 [`OF-DOG`](https://github.com/pei760730/OF-DOG) 的 FB 轉址解析更強，需要時可再 port 進來）。引擎完全相同，只把上游對接從 voc 換成 **TeaBus-VOC**。
> **待辦**：TeaBus-VOC 尚未建立 —— 開好後把它的 spreadsheet id 填進 `.env` 的 `GOOGLE_SHEET_ID`，並共用同一把 service account。下游契約見 `CLAUDE.md` §6。
...
訊息文字
  ...
  → dedupKey     連結→去重 key(平台:影片id 優先,抽不到退連結路徑;對齊 TeaBus-VOC sync._dedup_key)
  → 去重         讀參考池「連結」欄,同 key 視為重複(全表比對、無時間窗)
  → 重複:回提醒不寫入 / 不重複:append 進「參考池」(5 欄、平台小寫、加入日期 ISO)→ 回成功
```

> 寫入「參考池」5 欄 = TeaBus-VOC `schema.REFS`:`id`(留空)/`平台`(小寫碼)/`連結`/`挑`(留空)/`加入日期`(ISO)。
> 參考池是 TeaBus-VOC 永久池,bot 只 append 不刪列(prune 已隨暫存區退役)。
...
| `GOOGLE_SHEET_ID` | 試算表 ID(= TeaBus-VOC `VOC_SPREADSHEET_ID`) |
...
bot 是上游:**直接寫** Google 表「短影音進度N」的「參考池」分頁。下游 [TeaBus-VOC](https://github.com/pei760730/TeaBus-VOC) 直接消費(pick→待拍→完成);中間不再有「暫存區→sync-pool」複製層(2026-06-22 廢)。

- bot `GOOGLE_SHEET_ID` 要等於 TeaBus-VOC 的 `VOC_SPREADSHEET_ID`,憑證共用同一把 service account。
- 寫入「參考池」5 欄須與 TeaBus-VOC `schema.REFS` 完全對上(`id/平台/連結/挑/加入日期`,欄名+順序),平台用小寫碼。改欄名要兩 repo 一起改,由 `tests/contract.test.ts` 守。
- 參考池由 TeaBus-VOC `init-sheet` 擁有,bot 不自建/不改表頭(表頭不齊 fail-fast)。
...
3. 去重改連結 key(平台:影片id 收斂多形態),對齊 TeaBus-VOC;參考池永久池無時間窗
